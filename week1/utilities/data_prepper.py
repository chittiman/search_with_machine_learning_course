...
class DataPrepper:
    ...
    def __log_ltr_query_features(self, query_id, key, query_doc_ids, click_prior_query, no_results, terms_field="_id"):
        ...
        hits = response['hits']['hits']
        # hit_len = len(hits)
        # docs_len = len(query_doc_ids)
        # print (f"{hit_len=}  {docs_len=}" )
        #     # there should only be one hit
        doc_id_features = {}
        for doc_id,hit in zip(query_doc_ids, hits):
            features = hit['fields']['_ltrlog'][0]['log_entry']
            doc_id_features[doc_id] = features

            #num_features = len(features)
            #print (f"{num_features = }")

        feature_names = [feature_dict["name"] for feature_dict in features]    
        #print (f"{features=}")
        ...
        for doc_id in query_doc_ids:
            ...
            features = doc_id_features[doc_id]
            for feature_dict in features:
                feature_name = feature_dict["name"]
                feature_value = feature_dict.get("value", 0)
